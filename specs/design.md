...
- **Decision**: The admin surface is a **pi extension** shipped in an `admin/` workspace, loaded into the
  ...
  (`/dispatch status|pause|resume|runs|logs|budget|triggers|costs|settings|set|unset`) and one
  self-refreshing TUI overlay component with **five in-component views**: **LIST** — a framed,
  ...
  **what-if** (`w` cycles a shortlist of models seen in history plus declared counterfactual targets;
  ...
  scan is the kind of quiet load a dashboard must not add. The
